/**
 * ==============================================================================
 * CONTENT.JS - SINGLE SOURCE OF TRUTH FOR PERSONALIZING THIS WEBSITE
 * ==============================================================================
 * 
 * Rich Hindi, Hinglish, and English quotes & shayaris paired with your memories!
 * ==============================================================================
 */

export const content = {
  // 🌟 Main Profile Information
  sisterName: "My Dearest Sister",
  sisterNickname: "Chhoti / Didi / Partner in Crime",
  brotherName: "Your Brother",

  // 🎬 Hero Section
  hero: {
    badge: "Festive Edition • Raksha Bandhan 2026",
    greeting: "Hey Sis… ❤️",
    subGreeting: "This Raksha Bandhan…",
    tagline: "I made something special just for you.",
    taglineHindi: "यादों की मीठी चाशनी और प्यार के कच्चे धागे से सजी एक छोटी सी कोशिश ✨",
    taglineHinglish: "A digital surprise full of our best memories, shayaris & crazy laughs!",
    ctaText: "Start Our Journey ✨",
  },

  // 👑 Sister Introduction
  sisterIntro: {
    title: "The Person Behind So Many Memories ❤️",
    titleHindi: "यादों के हर पन्ने पर जिसका नाम लिखा है 💖",
    subtitle: "From sharing toys to sharing life's biggest secrets.",
    subtitleHinglish: "Bachpan ke jhagdon se le kar life ke deep conversations tak!",
    image: "/images/sister_main.jpg", // Festive celebration photo with matching sunshine yellow!
    quote: "She can annoy me in 5 seconds… and somehow make everything better in the next 5. 😂❤️",
    quoteHindi: "घर की सबसे प्यारी रौनक और मेरी सबसे पक्की दोस्त... मेरी बहना! 💖",
    quoteHinglish: "5 minute ladai, phir 50 minute tak uski gossip sunna — this is our unwritten sibling code! 😂❤️",
    badges: [
      { text: "👑 Queen of Drama", color: "bg-rose-500/20 text-rose-300 border-rose-500/30" },
      { text: "🍫 Certified Food Stealer", color: "bg-amber-500/20 text-amber-300 border-amber-500/30" },
      { text: "🛡️ Always My Protector", color: "bg-gold-500/20 text-gold-300 border-gold-500/30" },
      { text: "💖 Best Sister Ever", color: "bg-pink-500/20 text-pink-300 border-pink-500/30" },
    ]
  },

  // 📖 Memory Timeline (Our Little Story)
  timeline: [
    {
      chapter: "Chapter 01",
      year: "Childhood Days",
      title: "Our First Memories & Silly Fights",
      titleHindi: "बचपन की शरारतें और रिमोट की लड़ाई 📺",
      description: "From fighting over the smallest slice of cake and the TV remote, to crying when the other got scolded. We were partners in mischief from day one.",
      image: "/images/memory_2.jpg",
      quote: "Remember when we promised to keep each other's secrets from Mom and Dad?",
      quoteHindi: "वो बचपन की खट्टी-मीठी नोक-झोंक और रिमोट की लड़ाई... काश वो दिन फिर लौट आएं!",
      quoteHinglish: "Ek chocolate ke do hisse aur TV remote ke 100 jhagde — childhood was wild with you! 🍫📺"
    },
    {
      chapter: "Chapter 02",
      year: "Growing Up",
      title: "Somehow The Fights Became Memories",
      titleHindi: "लड़ते-लड़ते कब इतने बड़े हो गए 🌸",
      description: "As the years flew by, our endless arguments turned into late-night gossip sessions, homework rescues, and understanding each other with just a single glance.",
      image: "/images/memory_4.jpg",
      quote: "Growing up with you was the greatest blessing in disguise.",
      quoteHindi: "लड़ते-झगड़ते कब इतने बड़े हो गए पता ही नहीं चला, पर हमारा प्यार हर दिन और गहरा होता गया।",
      quoteHinglish: "Growing up made us realize ki chahe duniya idhar ki udhar ho jaye, tu hamesha meri best friend rahegi."
    },
    {
      chapter: "Chapter 03",
      year: "The Crazy Years",
      title: "Too Many Laughs & Secret Stories 😂",
      titleHindi: "हंसी के फव्वारे और सीक्रेट किस्से 🤣",
      description: "Too many inside jokes that make zero sense to anyone else. Too many unplanned trips, hilarious photoshoots, and pretending we have our lives together.",
      image: "/images/memory_5.jpg",
      quote: "We don't need words when our eye contact already says 'Did you see that?!'",
      quoteHindi: "दुनिया के सामने शरीफ और आपस में फुल ड्रामेबाज़ — ये है हमारी असली पहचान! 😂",
      quoteHinglish: "Our inside jokes make zero sense to the world, but we laugh until our stomach hurts! 🤣✨"
    },
    {
      chapter: "Chapter 04",
      year: "Today",
      title: "Different Paths, Same Bond",
      titleHindi: "रास्ते अलग, पर दिल का रिश्ता वही ❤️",
      description: "Life gets busy, schedules clash, and we might be in different places, but one call from you still feels like we are right back in our living room.",
      image: "/images/memory_1.jpg",
      quote: "Distance means nothing when someone means everything.",
      quoteHindi: "रास्ते अलग हुए, जिम्मेदारियां बढ़ीं, मगर तेरी एक आवाज़ आज भी दिल को सुकून दे जाती है।",
      quoteHinglish: "Busy schedules and different cities, but one call from you and sab pehle jaisa normal lagta hai. 📞❤️"
    },
    {
      chapter: "Chapter 05",
      year: "Forever",
      title: "No Matter Where Life Takes Us",
      titleHindi: "उम्र भर का साथ और अटूट विश्वास 🤝",
      description: "No matter how old we get, no matter how many changes life brings, you will always be my sister, my greatest cheerleader, and my family for life.",
      image: "/images/memory_3.jpg",
      quote: "Side by side or miles apart, we are sisters & brothers connected by heart.",
      quoteHindi: "फूलों का तारों का सबका कहना है, एक हज़ारों में मेरी बहना है! सारी उमर हमें संग रहना है।",
      quoteHinglish: "Forever partner in crime, forever my strength. Rakhi toh bas ek bahana hai, tu meri roz ki duaa hai."
    }
  ],

  // 💌 Things I Don't Say Often (Interactive Reveal Cards)
  unsaidWords: [
    {
      id: 1,
      teaser: "About your presence in my life...",
      teaserHindi: "मेरी ज़िंदगी में तुम्हारी अहमियत...",
      secret: "Thank you for always being there, even when I didn't know how to ask.",
      secretHindi: "जब भी ज़रूरत पड़ी, बिना कुछ कहे हमेशा मेरा साथ देने के लिए दिल से शुक्रिया बहना।",
      secretHinglish: "Bina kahe meri har tension samajh lene ke liye thank you. You're my silent guardian angel! 💖",
      icon: "Heart"
    },
    {
      id: 2,
      teaser: "A truth I don't admit often...",
      teaserHindi: "एक सच जो मैं कभी कबूल नहीं करता...",
      secret: "I may not say it every day, but I am extraordinarily lucky to have you as my sister.",
      secretHindi: "शायद रोज़ नहीं कह पाता, पर दुनिया में सबसे खुशनसीब हूं कि मुझे तेरे जैसी बहन मिली।",
      secretHinglish: "Roz nahi bolta, but honestly I'm the luckiest brother in the world to have you. 🌟",
      icon: "Sparkles"
    },
    {
      id: 3,
      teaser: "About our random moments...",
      teaserHindi: "हमारी वो छोटी-छोटी बातें...",
      secret: "You have a magic way of making the most ordinary, boring days feel special.",
      secretHindi: "तेरे पास वो जादू है जो बोरिंग से बोरिंग दिन को भी हंसी और खुशी से भर देता है।",
      secretHinglish: "Teri pagal harkatein aur random gossip kisi bhi boring din ko memorable bana deti hain! 😂",
      icon: "Smile"
    },
    {
      id: 4,
      teaser: "Whenever things get tough...",
      teaserHindi: "जब भी कोई मुश्किल वक्त आए...",
      secret: "Even when we disagree or argue, I will always, unconditionally, have your back.",
      secretHindi: "चाहे कितनी भी बहस हो जाए, मैं हमेशा तेरे साथ एक मजबूत चट्टान की तरह खड़ा रहूंगा।",
      secretHinglish: "Ladai chahe kitni bhi ho, agar kisi ne tujhe pareshan kiya toh uski khair nahi! 🛡️",
      icon: "Shield"
    },
    {
      id: 5,
      teaser: "About the future...",
      teaserHindi: "आने वाले कल के बारे में...",
      secret: "You'll always be my favorite sibling, no matter how old, busy, or far apart we become.",
      secretHindi: "उम्र चाहे कितनी भी हो जाए, तू हमेशा मेरी वही सबसे प्यारी और लाडली बहना रहेगी।",
      secretHinglish: "No matter how old or busy we get, you will forever be my number one person ❤️",
      icon: "Infinity"
    }
  ],

  // 💬 Sibling Shayari & Quotes Collection (Hindi & Hinglish)
  quotesShayari: [
    {
      id: "q1",
      category: "shayari",
      categoryLabel: "📜 दिल से हिंदी शायरी",
      quoteHindi: "कच्चे धागों से बनी पक्की डोर है राखी,\nप्यार और मीठी तकरार का नाम है राखी।\nखुशियां महके तेरे आंगन में सदा,\nभाई के दिल की यही दुआ है हर घड़ी।",
      quoteHinglish: "Kachhe dhaagon se bani pakki dor hai Rakhi, pyaar aur meethi takraar ka naam hai Rakhi! ❤️",
      image: "/images/memory_3.jpg",
      tag: "Rakhi Special",
      likes: 128
    },
    {
      id: "q2",
      category: "hinglish",
      categoryLabel: "💬 बिंदास Hinglish",
      quoteHindi: "घर की सबसे बड़ी ड्रामेबाज़, पर दिल की सबसे साफ़... मेरी प्यारी बहना!",
      quoteHinglish: "Ghar ki sabse badi dramebaaz, lekin dil ki sabse saaf — meri behen jaisi koi nahi! 👑💅",
      image: "/images/sister_main.jpg",
      tag: "Drama Queen",
      likes: 95
    },
    {
      id: "q3",
      category: "shayari",
      categoryLabel: "📜 दिल से हिंदी शायरी",
      quoteHindi: "फूलों का तारों का सबका कहना है,\nएक हज़ारों में मेरी बहना है!\nसारी उमर हमें संग रहना है।",
      quoteHinglish: "Phoolon ka taaron ka sabka kehna hai, ek hazaron mein meri behna hai! 🌸✨",
      image: "/images/memory_1.jpg",
      tag: "Timeless Classic",
      likes: 154
    },
    {
      id: "q4",
      category: "masti",
      categoryLabel: "😂 सिबलिंग मस्ती",
      quoteHindi: "5 मिनट की लड़ाई, फिर 50 मिनट की गॉसिप — यही है हमारा असली सिबलिंग कोड!",
      quoteHinglish: "Ladai chahe kitni bhi ho, par sabse pehle koi nayi baat batane ke liye call tujhe hi lagta hai! 📞😂",
      image: "/images/memory_2.jpg",
      tag: "BFF & Gossip",
      likes: 88
    },
    {
      id: "q5",
      category: "rakhi",
      categoryLabel: "🧵 रक्षा बंधन स्पेशल",
      quoteHindi: "रेशम की डोरी, चंदन का टीका, सावन की फुहार,\nमांगूं बस यही दुआ, खुशियों से भरा रहे बहना का संसार!",
      quoteHinglish: "Aarti ki thaali, meethi si smile aur hamesha protection ka vaada — Happy Rakhi! 🪔✨",
      image: "/images/memory_8.jpg",
      tag: "Festive Blessings",
      likes: 112
    },
    {
      id: "q6",
      category: "hinglish",
      categoryLabel: "💬 बिंदास Hinglish",
      quoteHindi: "तू मेरी वो बेस्ट फ्रेंड है जिसे चुनने का ऑप्शन मुझे कभी नहीं मिला... और शुक्र है नहीं मिला! ❤️",
      quoteHinglish: "You are the best friend I was stuck with by birth, and honestly, best deal ever! 🤝✨",
      image: "/images/memory_4.jpg",
      tag: "Sibling Squad",
      likes: 104
    },
    {
      id: "q7",
      category: "shayari",
      categoryLabel: "📜 दिल से हिंदी शायरी",
      quoteHindi: "चेहरे पर मुस्कान और आंखों में चमक रहे सदा,\nगम की कोई परछाईं भी तेरे पास न आए कभी खुदा।",
      quoteHinglish: "May your smile stay radiant forever, and every day bring you endless sunshine! 🌹🏔️",
      image: "/images/memory_6.jpg",
      tag: "Pure Blessings",
      likes: 76
    },
    {
      id: "q8",
      category: "masti",
      categoryLabel: "😂 सिबलिंग मस्ती",
      quoteHindi: "कहने को बहुत शांत और शरीफ बनती है, पर घर के सारे कांडों की असली मास्टरमाइंड यही है! 🤫😇",
      quoteHinglish: "Acting like the sweet innocent angel in front of parents, but partner-in-crime with me! 🤫😇",
      image: "/images/memory_5.jpg",
      tag: "Secret Mastermind",
      likes: 119
    },
    {
      id: "q9",
      category: "rakhi",
      categoryLabel: "🧵 रक्षा बंधन स्पेशल",
      quoteHindi: "कलाई पर रेशम का धागा, माथे पर चन्दन का टीका,\nबहन के प्यार के आगे दुनिया का हर सुख फीका!",
      quoteHinglish: "Kalai par resham ka dhaaga aur behen ka dulaar — Rakhi bond forever! 🧵❤️",
      image: "/images/memory_11.jpg",
      tag: "Rakhi Vows",
      likes: 142
    },
    {
      id: "q10",
      category: "masti",
      categoryLabel: "😂 सिबलिंग मस्ती",
      quoteHindi: "हम जब भी साथ होते हैं, तो घर में हंसी और ड्रामे का तूफान आ जाता है! ⚡😂",
      quoteHinglish: "The ultimate sibling trio — full volume, 100% drama, infinite laughter! ⚡😂",
      image: "/images/memory_13.jpg",
      tag: "Sibling Squad",
      likes: 135
    },
    {
      id: "q11",
      category: "shayari",
      categoryLabel: "📜 दिल से हिंदी शायरी",
      quoteHindi: "अपनों का साथ हो तो हर दिन त्योहार बन जाता है,\nपरिवार की खुशियों से ही हर आंगन महक जाता है।",
      quoteHinglish: "Family vibes are the best vibes — togetherness that warms the heart! 🏡✨",
      image: "/images/memory_12.jpg",
      tag: "Parivar",
      likes: 167
    },
    {
      id: "q12",
      category: "hinglish",
      categoryLabel: "💬 बिंदास Hinglish",
      quoteHindi: "सादगी में भी एक शाही अंदाज़ है, मेरी बहना पर मुझे हमेशा नाज़ है! 👑✨",
      quoteHinglish: "Elegance, grace, and an evergreen sweet smile — meri behna sabse best! 🌿🌸",
      image: "/images/memory_14.jpg",
      tag: "Evergreen Smile",
      likes: 121
    }
  ],

  // 😂 Fun Sister Section (Playful Quirks)
  funTraits: [
    {
      emoji: "❤️",
      title: "Always Caring",
      titleHindi: "दिखावे की डांट, दिल से प्यार",
      desc: "Pretends not to care, but asks 10 questions to make sure I ate and reached safely.",
      descHindi: "सामने से अकड़ दिखाएगी, पर पीछे से 10 बार पूछेगी कि खाना खाया या नहीं!"
    },
    {
      emoji: "😂",
      title: "Professional Irritator",
      titleHindi: "परेशान करने में गोल्ड मेडलिस्ट",
      desc: "Has a PhD in annoying me at the exact moment I am trying to focus.",
      descHindi: "जब मैं सबसे ज़्यादा बिज़ी होऊं, तभी कमरे में आकर ड्रामे शुरू करना इसका नियम है।"
    },
    {
      emoji: "🍫",
      title: "Midnight Food Thief",
      titleHindi: "आधी रात की स्नैक चोर",
      desc: "Says 'I am not hungry', then eats 70% of whatever snack I ordered.",
      descHindi: "बोलेगी 'मुझे भूख नहीं है', फिर मेरे हिस्से का 80% पिज्जा या मैगी चट कर जाएगी!"
    },
    {
      emoji: "👀",
      title: "Knows All Secrets",
      titleHindi: "सीबीआई से तेज़ नज़रें",
      desc: "Can read my face in 0.2 seconds and knows exactly what trouble I am in.",
      descHindi: "एक नज़र में पहचान लेती है कि आज क्या गड़बड़ हुई है।"
    },
    {
      emoji: "😎",
      title: "Chief Family Advisor",
      titleHindi: "घर की मुख्य सलाहकार",
      desc: "Somehow knows all the family updates before the news channel does.",
      descHindi: "सारे रिश्तेदारों और दोस्तों की लेटेस्ट खबरें सबसे पहले इसी के पास आती हैं।"
    },
    {
      emoji: "👑",
      title: "Queen of the House",
      titleHindi: "घर की असली हुकूमत",
      desc: "Wins 100% of arguments by simply declaring 'Because I said so!'",
      descHindi: "'क्योंकि मैंने कहा है!' बोलकर हर बहस 2 सेकंड में जीत लेती है।"
    },
    {
      emoji: "🫶",
      title: "Always There",
      titleHindi: "हर मोड़ पर पक्की ढाल",
      desc: "The one person I can always count on, through sunshine and storm.",
      descHindi: "दुनिया कुछ भी कहे, ये हमेशा मेरे साथ चट्टान की तरह खड़ी रहती है।"
    }
  ],

  // 🎵 Spotify Playlist Details
  spotify: {
    title: "Our Raksha Bandhan Soundtrack 🎵",
    titleHindi: "यादों के सुरमई तराने 🎶",
    subtitle: "Some memories sound so much sweeter with music.",
    subtitleHinglish: "Press play and let every song bring back our childhood laughs!",
    playlistId: "7DILEiQF93PlU7eKMI2FIt",
    playlistUrl: "https://open.spotify.com/playlist/7DILEiQF93PlU7eKMI2FIt",
    caption: "Press play, turn up the volume, and let every song bring back our favorite smiles. ❤️"
  },

  // 📸 Memory Photo Gallery (Masonry with Lightbox & Image-Based Hindi/Hinglish Quotes)
  gallery: [
    {
      id: 1,
      image: "/images/memory_11.jpg",
      caption: "The sacred Raksha Bandhan ritual — brother-sister love, auspicious thali, and lifelong promises 🧵❤️",
      tag: "Rakhi Bandhan",
      category: "rakhi",
      quoteHindi: "रेशम की डोरी और बहन का दुलार, खुशियों से महके ये पावन त्योहार! 🧵💖",
      quoteHinglish: "Haath par bandhi rakhi aur dil mein hamesha protection ka vaada — Happy Raksha Bandhan! 🪔✨"
    },
    {
      id: 2,
      image: "/images/memory_13.jpg",
      caption: "Festive sibling trio — tilak on forehead, bright smiles, and pure bond 🪔✨",
      tag: "Sibling Trio",
      category: "squad",
      quoteHindi: "माथे पर लाल तिलक और बहनों की दुआएं — हमारी इस तिकड़ी पर कभी कोई आंच न आए! 🪔✨",
      quoteHinglish: "Brother-sister trio power! Tilak on point and endless smiles with my favorite girls! 💫❤️"
    },
    {
      id: 3,
      image: "/images/memory_12.jpg",
      caption: "Full family celebration — warmth, togetherness, and cherished blessings 🏡❤️",
      tag: "Family Love",
      category: "family",
      quoteHindi: "परिवार का प्यार और अपनों का साथ — हर पल को ख़ास बना देता है ये प्यारा संसार! 👨‍👩‍👧‍👦✨",
      quoteHinglish: "When the whole family gathers together, every moment turns into a lifelong celebration! 🏡❤️"
    },
    {
      id: 4,
      image: "/images/memory_15.jpg",
      caption: "Radiant sister duo — unbreakable bond, style, and smiles together 👭✨",
      tag: "Sister Duo",
      category: "sister",
      quoteHindi: "दो बहनों की ये प्यारी जोड़ी, जैसे खुशियों की कोई मीठी लोरी! 👭✨",
      quoteHinglish: "Two radiant sisters, infinite memories, and unmatched bond! Power duo forever! 💖🌿"
    },
    {
      id: 5,
      image: "/images/memory_14.jpg",
      caption: "Graceful sister portrait amidst green serenity and gentle sunshine 🌿🌸",
      tag: "Sister Portrait",
      category: "sister",
      quoteHindi: "प्रकृति की गोद में खिली मुस्कान, सादगी और खूबसूरती की मिसाल मेरी बहना! 🌿🌸",
      quoteHinglish: "Surrounded by nature with that bright signature smile — pure elegance & grace! 🍃✨"
    },
    {
      id: 6,
      image: "/images/sister_main.jpg",
      caption: "Vibrant Haldi festivities & sibling love in matching sunshine yellow 💛✨",
      tag: "Haldi Celebrations",
      category: "festive",
      quoteHindi: "हल्दी का रंग और तेरी प्यारी सी मुस्कान — खुशियों से महकता रहे तेरा हर एक पल! 💛✨",
      quoteHinglish: "Haldi ka glow ek taraf, aur teri nautanki ek taraf — full festive queen vibes! 🌟💛"
    },
    {
      id: 7,
      image: "/images/memory_3.jpg",
      caption: "The sacred Raksha Bandhan aarti — prayers, blessings, and forever bonds 🧵❤️",
      tag: "Rakhi Aarti",
      category: "rakhi",
      quoteHindi: "रेशम की डोरी में बंधा है भाई-बहन का सच्चा प्यार, खुशियों से महके बहना का ये पावन त्योहार! 🧵❤️",
      quoteHinglish: "Aarti ki thaali, meethi si smile aur hamesha protection ka vaada — Happy Rakhi! 🌸✨"
    },
    {
      id: 8,
      image: "/images/memory_1.jpg",
      caption: "Festive family celebrations & royal traditional vibes ✨",
      tag: "Family Festivities",
      category: "family",
      quoteHindi: "परिवार की रौनक और हर महफ़िल की जान — सदा चमकती रहे मेरी प्यारी बहना की शान! 👑",
      quoteHinglish: "Traditional attire mein royal lagti hai, par harkatein wahi bachpan wali! 😎👑"
    },
    {
      id: 9,
      image: "/images/memory_6.jpg",
      caption: "Mountain sunshine, blooming roses, and beautiful scenery with you 🌸🏔️",
      tag: "Nature & Roses",
      category: "sister",
      quoteHindi: "फूलों जैसी कोमल और वादियों जैसी खूबसूरत — रब से बस यही दुआ है कि तू हमेशा मुस्कुराए! 🌸🏔️",
      quoteHinglish: "Pahadon ki thandi hawa aur tere saath crazy photoshoots — priceless memories! 🌹⛰️"
    },
    {
      id: 10,
      image: "/images/memory_7.jpg",
      caption: "Vibrant festivities, colorful canopies, and adorable family smiles ⛱️✨",
      tag: "Festive Joy",
      category: "festive",
      quoteHindi: "रंग-बिरंगी खुशियों का ये सवेरा, सदा रोशन रहे ज़िंदगी का बसेरा! ⛱️✨",
      quoteHinglish: "Rang birangi yaadein aur non-stop masti — festive vibes hit different with you! 🌈🎉"
    },
    {
      id: 11,
      image: "/images/memory_8.jpg",
      caption: "Golden traditions, blessings, and treasured family moments 💛",
      tag: "Golden Traditions",
      category: "festive",
      quoteHindi: "रिश्तों की मिठास और अपनों का दुलार — भगवान करे हमेशा बना रहे खुशियों का संसार! 💛",
      quoteHinglish: "Desi rituals, candid laughs aur golden moments that we'll cherish forever! 💫✨"
    },
    {
      id: 12,
      image: "/images/memory_9.jpg",
      caption: "Grace, elegance, and timeless beauty 👑✨",
      tag: "Royal Grace",
      category: "sister",
      quoteHindi: "सादगी में भी एक शाही अंदाज़ है, मेरी बहना पर मुझे हमेशा नाज़ है! 👑✨",
      quoteHinglish: "Royal aesthetic on point! You bring so much grace and charm everywhere you go. 💅✨"
    },
    {
      id: 13,
      image: "/images/memory_10.jpg",
      caption: "Sweet smiles, peaceful evenings, and golden terrace moments 💙",
      tag: "Sweet Moments",
      category: "sister",
      quoteHindi: "शाम की ठंडी हवा और अपनों की बातें — कितनी प्यारी होती हैं छत पर बीती ये रातें! 🌙",
      quoteHinglish: "Terrace pe baith kar life ki deep baatein aur chai ki chuski — best sibling therapy! ☕🌆"
    },
    {
      id: 14,
      image: "/images/memory_2.jpg",
      caption: "Terrace masti, crazy selfies, and endless laughter with you 😂❤️",
      tag: "Fun Times",
      category: "squad",
      quoteHindi: "जब हम दोनों साथ हों तो हंसी नहीं रुकती, भाई-बहन की ये मस्ती कभी कम नहीं होती! 😂❤️",
      quoteHinglish: "Gallery mein 100 goofy pictures aur zero serious ones — that is pure sibling goals! 🤪📸"
    },
    {
      id: 15,
      image: "/images/memory_4.jpg",
      caption: "The Sibling Squad — together through every lockdown and adventure 😎",
      tag: "Sibling Squad",
      category: "squad",
      quoteHindi: "एक साथ जब खड़े हों हम, तो हर मुश्किल हो जाती है कम — बेस्ट सिबलिंग स्क्वॉड! 😎",
      quoteHinglish: "Squad goals unlocked! Chahe koi bhi problem ho, hum sab mil kar sambhal lenge! 🤝💥"
    },
    {
      id: 16,
      image: "/images/memory_5.jpg",
      caption: "Finding inner peace before starting the next sibling argument 🧘‍♀️😂",
      tag: "Peace & Masti",
      category: "sister",
      quoteHindi: "2 मिनट की शांति, फिर वही तूफान — पर इसी पागलपन में बसती है मेरी जान! 🧘‍♀️😂",
      quoteHinglish: "Acting like the calm and innocent one, but mastermind of all ghar ke pranks! 🤫😇"
    }
  ],

  // 🧵 Interactive Rakhi Section
  rakhi: {
    title: "A Rakhi Is More Than Just A Thread…",
    titleHindi: "रेशम की डोरी, अटूट विश्वास 🧵❤️",
    quote: "It's a sacred thread woven with prayers, childhood laughter, silent promises, and a bond that time or distance can never break.",
    quoteHindi: "कच्चे धागों से बनी पक्की डोर है राखी, प्यार और मीठी तकरार का नाम है राखी। भाई की कलाई पर बंधी बहना की दुआ है राखी!",
    quoteHinglish: "A simple sacred thread with infinite love & protection — hamesha tere saath khada rahunga! 🌸",
    sloka: "येन बद्धो बली राजा दानवेन्द्रो महाबलः। तेन त्वां प्रतिबध्नामि रक्षे मा चल मा चल॥",
    buttonText: "Tie The Rakhi 🧵❤️",
    buttonTextHindi: "राखी बांधें 🧵❤️",
    successHeading: "Bond Locked Forever ❤️",
    successHeadingHindi: "प्यार का अटूट बंधन सदा अमर रहे ❤️",
    successSub: "May your life be filled with endless joy, radiant health, and boundless happiness. Your brother is always with you!",
    successSubHindi: "सदा मुस्कुराती रहो मेरी प्यारी बहना, खुशियों से महकता रहे तुम्हारा हर आशियाना!"
  },

  // 🎁 Final Surprise & Emotional Finale
  surprise: {
    title: "One Last Surprise… 🎁",
    titleHindi: "दिल से एक आखिरी ख़ास तोहफा… 🎁",
    subtitle: "A heartfelt promise packaged with love.",
    subtitleHinglish: "Dil se nikli hui duayein aur sadiyon ka pyaar.",
    buttonText: "Open Your Gift ❤️",
    buttonTextHindi: "तोहफा खोलें ✨",
    heading: "Happy Raksha Bandhan, Sis! ❤️",
    headingHindi: "रक्षा बंधन की ढेर सारी शुभकामनाएं बहना! 💖",
    letter: [
      "No matter how much we grow up, no matter where our individual paths lead us in this world, you will always be my sister, my first best friend, and one of my most cherished people.",
      "Thank you for being my constant, for your warm hugs, your endless patience, and even your drama. Life wouldn't be half as vibrant without you in it.",
      "I promise to always stand by you, protect your smile, and cheer for you louder than anyone else in the room."
    ],
    letterHindi: [
      "बचपन की यादों से लेकर आज तक, तू हमेशा मेरी सबसे बड़ी ताकत और सबसे प्यारी दोस्त रही है।",
      "तेरी हंसी, तेरी ड्रामेबाज़ी और तेरा बिना वजह गुस्सा होना — सब कुछ मेरे दिल के बहुत करीब है। ज़िंदगी तेरे बिना इतनी खूबसूरत कभी नहीं हो सकती थी।",
      "वादा है मेरा कि ज़िंदगी के हर मोड़ पर, हर परिस्थिति में तेरा भाई हमेशा तेरे साथ चट्टान की तरह खड़ा रहेगा।"
    ],
    letterHinglish: [
      "Chahe hum kitne bhi bade ho jayein, our bond will forever stay pure, crazy, and unconditional.",
      "Thank you for being my constant cheerleader, my secret keeper, and my favorite troublemaker.",
      "I promise to always protect your smile and stand by you in every step of life."
    ],
    finalTagline: "Love you always & forever. 🫶",
    finalTaglineHindi: "सदा खुश रहो और मुस्कुराती रहो! ❤️",
    credits: "Made with love, cherished memories, and code. 💻❤️",
    finalPhoto: "/images/memory_1.jpg",
    finalPhotoCaption: "Forever my sister. Forever my family. ❤️",
    finalPhotoCaptionHindi: "एक हज़ारों में मेरी बहना है! ❤️✨"
  }
};
